...
export default function OpeningDetail({ opening, neighbours, onSelect, onCopy, onClose, variationNotes, variationNoteError, onRequestVariationNotes, onRetryVariationNotes }: OpeningDetailProps) {
  ...
  return <div className="detail-content"><button className="detail-close" onClick={onClose} aria-label="關閉開局詳情">×</button>
    ...
    </div>{opening.variations.length === 0 && <aside className="empty-variations"><span aria-hidden="true">✓</span><div><b>目前沒有同名、可靠歸屬的具名子變例</b><p>這不是資料漏掉；為了不把相似棋路誤標成同一變例，只顯示官方辨識棋路。實戰續行請看下方「重要招法｜接下來怎麼下」。</p></div></aside>}</section>
