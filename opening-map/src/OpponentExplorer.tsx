...
export default function OpponentExplorer({ data }: { data: OpeningMapData }) {
  ...
  return <section className="opponent-explorer"><div className="concept-heading"><p className="eyebrow">PRACTICE MATCH</p><h2>選擇對手等級與顏色</h2><p>你可以執白或執黑；對手會依選擇的強度自動回棋，也能隨時撤回上一回合。</p></div>
    <div className="match-mode-selector" aria-label="建立對局">
      <button className={matchMode === "normal" ? "active" : ""} aria-pressed={matchMode === "normal"} onClick={() => { setMatchMode("normal"); setBlindStockfish(false); setGame((value) => value + 1); }}><span>♟</span><div><small>建立對局</small><b>一般對局</b><p>完整棋盤，原本的對手練習功能。</p></div></button>
      <button className={matchMode === "blind" ? "active blind" : ""} aria-pressed={matchMode === "blind"} onClick={() => { setMatchMode("blind"); setBlindStockfish(false); setGame((value) => value + 1); }}><span>◌</span><div><small>建立對局</small><b>盲棋對局</b><p>棋子隱藏，只保留對手最後一步的橘色框。</p></div><i>Live Board<br />預設折疊</i></button>
    </div>
    ...
    <div className="opponent-layout"><div className="opponent-levels">{opponentLevels.map((item) => <button className={level === item.value ? "active" : ""} aria-pressed={level === item.value} key={item.value} onClick={() => chooseLevel(item)}><span>{item.value}</span><div><b>{item.name} · {item.opponent}</b><small>{item.rating}</small><p>{item.text}</p></div></button>)}<OpeningRecognition data={data} position={position} /></div>
