...
export function App() {
  ...
  return <main>
    ...
    {query.trim() && selected && <aside className="detail modal-detail" aria-live="polite"><OpeningDetail key={selected.id} opening={selected} neighbours={neighbours} onSelect={selectOpening} onCopy={copyLine} onClose={() => setSelectedId(null)} /></aside>}
  </main>;
}
...
function OpeningDetail({ opening, neighbours, onSelect, onCopy, onClose }: { opening: Opening; neighbours: Opening[]; onSelect: (id: string) => void; onCopy: (line: string) => void; onClose: () => void }) {
...
function playersForOpening(opening: Opening) { const text = `${opening.title_zh} ${opening.title_en}`; return playerProfiles.filter((player) => player.keys.some((key) => text.includes(key))).slice(0, 4); }
...
function corePlanSquares(plan: string) { return [...new Set(plan.match(/\b[a-h][1-8]\b/gi) ?? [])]; }
