...
export default function TranspositionExplorer({ nodes, groups, onSelect }: { nodes: Opening[]; groups: TranspositionGroup[]; onSelect: (id: string) => void }) {
  ...
  return <div className="transposition-explorer">
    ...
    <div className="transposition-layout">
      <nav ref={groupListRef} className="transposition-group-list" role="tablist" aria-label="體系轉換群組">{groups.map((item, index) => <button type="button" role="tab" id={`transposition-tab-${item.id}`} aria-controls="transposition-group-detail" aria-selected={item.id === group.id} tabIndex={item.id === group.id ? 0 : -1} className={item.id === group.id ? "active" : ""} key={item.id} onClick={() => setActiveGroup(item.id)} onKeyDown={(event) => moveGroup(event, index)}><span>{item.source === "curated" ? "精選" : "官方"}</span><b>{item.title}</b><small>{item.memberIds.length} 個體系・{item.routes.length} 條走序</small></button>)}</nav>
      <section className="transposition-detail" id="transposition-group-detail" role="tabpanel" aria-labelledby={`transposition-tab-${group.id}`} aria-live="polite"><header><div><span className="exact-badge">⇄ 精確同局面</span><h3>{group.title}</h3><p>{group.summary}</p></div><FenPositionPreview fen={group.targetFen} label="共同目標局面" /></header>
