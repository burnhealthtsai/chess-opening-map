...
function puzzleRound(puzzle: PuzzleSummary) {
  return Number(puzzle.title.match(/第\s*(\d+)\s*回合/)?.[1] ?? 0);
}

function inPuzzleGroup(puzzle: PuzzleSummary, group: PuzzleGroup) {
...
export default function PuzzleExplorer() {
  ...
  const selectedSummary = catalog?.puzzles.find((puzzle) => puzzle.id === selectedId) ?? null;
